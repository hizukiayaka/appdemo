...
typedef struct
{
  ...
  GtkWidget *video_widget1;

  GstElement *pipeline;
  GstElement *pipeline1;
  GstVideoOverlay *overlay;
  GstVideoOverlay *overlay1;
...
static GstBusSyncReply
bus_sync_handler1 (GstBus * bus, GstMessage * message, gpointer user_data)
{
  DemoApp *d = user_data;
  ...
  if (gst_is_wayland_display_handle_need_context_message (message)) {
    ...
    display = gtk_widget_get_display (d->video_widget1);
    display_handle = gdk_wayland_display_get_wl_display (display);
    context = gst_wayland_display_handle_context_new (display_handle);
    gst_element_set_context (GST_ELEMENT (GST_MESSAGE_SRC (message)), context);
     ...
     * playbin resets the window handle and render_rectangle after restarting
     * playback and the actual window size is lost */
    d->overlay1 = GST_VIDEO_OVERLAY (GST_MESSAGE_SRC (message));

    gtk_widget_get_allocation (d->video_widget1, &allocation);
    window = gtk_widget_get_window (d->video_widget1);
    window_handle = gdk_wayland_window_get_wl_surface (window);

    g_print ("setting window handle and size (%d x %d)\n",
        allocation.width, allocation.height);

    gst_video_overlay_set_window_handle (d->overlay1, (guintptr) window_handle);
...
static gboolean
video_widget_draw_cb1 (GtkWidget * widget, cairo_t * cr, gpointer user_data)
{
  DemoApp *d = user_data;
  GtkAllocation allocation;
...
build_window (DemoApp * d)
{
  ...
  gtk_window_fullscreen (GTK_WINDOW (gtk_builder_get_object (builder, "window")));
  ...
  d->video_widget1 = GTK_WIDGET (gtk_builder_get_object (builder, "videoarea1"));
  g_signal_connect (d->video_widget1, "draw",
      G_CALLBACK (video_widget_draw_cb1), d);
...
main (int argc, char **argv)
{
  ...
  if (argc > 1) {
    ...
    d->pipeline1 = gst_parse_launch ("playbin video-sink=waylandsink", NULL);
    g_object_set (d->pipeline1, "uri", argv[d->current_uri + 1], NULL);
    /* enable looping */
    g_signal_connect (d->pipeline, "about-to-finish",
        G_CALLBACK (on_about_to_finish), d);
    g_signal_connect (d->pipeline1, "about-to-finish",
        G_CALLBACK (on_about_to_finish), d);
  } else {
    if (live) {
  ...
  bus = gst_pipeline_get_bus (GST_PIPELINE (d->pipeline1));
  gst_bus_add_signal_watch (bus);
  g_signal_connect (bus, "message::error", G_CALLBACK (error_cb), d);
  gst_bus_set_sync_handler (bus, bus_sync_handler1, d, NULL);
  gst_object_unref (bus);
  ...
  gst_element_set_state (d->pipeline, GST_STATE_NULL);
  gst_element_set_state (d->pipeline1, GST_STATE_NULL);
